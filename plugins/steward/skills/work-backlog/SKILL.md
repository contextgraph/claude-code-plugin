...
- `action: "resolve"` — close an item whose deliverable is **not** a pull request, with `outcome: "verified"` (a valid concern checked and found already-covered — a steward win) or `outcome: "finding"` (an investigation that recorded an answer). Use this for already-covered work instead of dismissing it.
